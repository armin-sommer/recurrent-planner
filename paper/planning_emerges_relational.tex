...
graph. The value of each state is computed in an \emph{amortized} way --- a fixed
value field, decodable from the cells and not re-propagated over thinking steps ---
and the thinking loop performs the complementary half, \emph{policy improvement}:
it iteratively refines the decision read off that value toward the greedy action.
Amortized evaluation and iterated improvement are, together, generalized policy
iteration in latent space.
\end{abstract}
...
experiments, is that the thinking loop is the \emph{policy-improvement} half of
policy iteration. The value of each state --- decodable from its cell --- is
computed in an \emph{amortized} way: a fixed value field, set within the first
tick and maintained across environment steps by the critic's temporal-difference
learning, \emph{not} re-propagated over thinking ticks. What the thinking ticks do
is iterate \emph{policy improvement}: the localized head reads the greedy action
over that fixed value, and each additional tick refines this decision toward the
greedy choice, converging the action --- most for the hardest states, which need
the most improvement. Amortized evaluation (the compiled operator together with
TD across environment steps) and iterated improvement (the thinking loop) together
constitute \emph{generalized policy iteration} on a graph the network has itself
learned --- a generic relational recurrence, given only RL signal.
...
(ii) We show that, in this configuration, the thinking loop is the
\emph{policy-improvement} half of generalized policy iteration
(Section~\ref{sec:planning}, Proposition~\ref{prop:improvement}): the state value
is computed amortized (a fixed value field), and additional thinking refines the
decision read from it toward the greedy action --- improvement unrolled in latent
space over an amortized evaluation.
...
\section{Emergent Planning as Policy Improvement over an Amortized Value}
\label{sec:planning}

We now characterize what the update \eqref{eq:update} computes in a bound,
localized configuration. The picture is a clean split: the per-state \emph{value}
is computed in an amortized way --- a fixed field --- and the thinking loop is the
\emph{policy-improvement} half of policy iteration, iteratively refining the
decision read off that value.

\paragraph{An amortized value.} The cell $\sigma(s)$ carries a \emph{value}
$v(s)$, decodable from its content by the critic's fixed readout,
$v(s)=\rho_V\!\bigl(h(\sigma(s))\bigr)$ (Def.~\ref{def:localized}; empirically $V$
is linearly decodable from the cell latent, \S\ref{sec:emp-binding}), alongside a
reward channel holding $\mathcal R(s)$. This value is the fixed point of a learned
graph operator $\mathcal B$ over $\mathcal N$ --- a discounted aggregate of the
rewards reachable along the graph, the resolvent $(I-\gamma\mathcal B)^{-1}r$
(Assumption~\ref{ass:value}). Crucially, the network does \emph{not} re-derive it
across thinking ticks: it is computed \emph{amortized} --- set within the first
tick and held fixed thereafter, and maintained across environment steps by the
critic's temporal-difference learning (verified in \S\ref{sec:emp-planning}). The
value is the substrate the loop plans over, not the loop's per-tick product.
...
network realises $v$ \emph{amortized} (in essentially one compiled step), rather
than by iterating $\mathcal B$ over thinking ticks.
\end{assumption}

\paragraph{The thinking loop improves the policy.} The actor is localized and
\emph{greedy-consistent}: it reads, at the current cell, the action whose
out-neighbour has the highest backed-up value. Writing $g^{k}(s)$ for the decision
the head would commit at thinking step $k$, the thinking loop refines it toward the
greedy choice over the fixed value,
\begin{equation}
  g^{k+1}(s)\;=\;\operatorname*{arg\,max}_{s':(s,s')\in\mathcal N}
  \bigl[\,\mathcal R(s')+\gamma\,v(s')\,\bigr],
  \qquad
  \pi(\cdot\mid s_t)=\rho_\pi\bigl(h^{K}(\sigma(s_t))\bigr),
  \label{eq:improvement}
\end{equation}
the $\max$ that policy iteration's improvement step applies --- here over the
amortized value. The selection lives at the head; the value channel is fixed, and
what moves across thinking ticks is this decision.
...
\begin{proposition}[The loop is policy improvement]
\label{prop:improvement}
Under Assumption~\ref{ass:value} with a greedy-consistent localized head, the
...
not yet greedy for $v$ \citep[multi-step policy improvement;][]{efroni2018beyond},
converging to the greedy policy with respect to $v$. The improvement is largest
where the decision is hardest --- states far from reward, which take the most ticks
to settle. In the sparse terminal-reward setting, once $K$ exceeds the
graph-distance from $s$ to $s_{\text{goal}}$, $g^{K}(s)$ lies on a $\gamma$-optimal
path regardless of the accuracy of $v$ beyond its sign.
\end{proposition}

\paragraph{This is generalized policy iteration --- with evaluation amortized.}
Generalized policy iteration is a value estimate and a policy improving against one
another \citep{sutton2018reinforcement}; here the split is explicit.
\emph{Evaluation is amortized}: the value field is compiled into the forward pass
(one operator step, Assumption~\ref{ass:value}) and maintained across environment
steps by the critic's TD updates --- it does not iterate over thinking ticks.
\emph{Improvement is the thinking loop}: each tick refines the decision toward
greedy over that fixed value (\eqref{eq:improvement}), the budget $K$ setting how
far the decision is improved. A generic relational recurrence, given only RL
signal, thereby runs the improvement half of policy iteration over a value it has
learned to compute in one shot.
...
action, and explain why extra test-time compute helps
\citep{guez2019investigation}; the same forming-over-layers signature appears in
deep Sokoban ResNets \citep{taufeeque2024planning}. That is the \emph{decision}
being improved over ticks --- not the value, which is amortized. Where the routing
is local (convolutional) the improvement propagates as an outward plan frontier;
where it is dense (attention) the same improvement is global, surfacing as the
settling action at the head (\S\ref{sec:empirics}). The improvement is consolidated
rather than recomputed: the actor exposes $g^{K}(s_t)$ to the policy gradient,
which shifts parameters toward the improved policy, and across environment steps
the critic re-evaluates --- closing the loop.
