...
>
> **AUTONOMOUS OVERNIGHT — LIVE (2026-06-11T00:59Z):** `/workspace/claude_experiments/overnight.sh` (setsid, node-side, survives any Claude-session restart) drives the chain: wait matrix → `bash driver_attn_200m.sh` (6 attn arms `attn_{masked,dense,softmax,plain,vn,shallow}_200m` ← `sokoban_drc_attn_3_3 / _nomask / _softmax / _plain / _vn / _1_1`) → wait → `cleanba.load_and_eval load_other_run=<arm> only_last_checkpoint=true` (`planning_eval_envs`: valid_medium, steps_to_think→48, 4000 levels) on matrix then attn finals → writes `$EXP/STATUS_*` flags + `*_metrics_dict.pkl` inside each final `cp_*`. Watch: `ls $EXP/STATUS_*`; `tail $EXP/logs/overnight.log`. Configs pushed at `74d89b2`.
> - **Local background monitors keep dying on app restart** — that's why orchestration is node-side; the node self-drives, no local poller needed for compute. (SSH connection itself is fine.)
> - **RESULTS push is LOCAL-only (node has NO git creds):** when `STATUS_EVAL_DONE`/`ALL_DONE` appears, pull each arm's `cp_*/valid_medium_metrics_dict.pkl` + read_eval curves → build `results/{data,figures,RESULTS.md}` → commit + push from the local machine. Preliminary results buildable from the during-training `@0/@8` offline-wandb curves once `STATUS_MATRIX_DONE`.
>
> **`local_max` REDO PLAN (run when GPUs free post-chain; BOUNDED — a negative result is a valid finding, do NOT force it to work):** the 200M run OSCILLATES from ~130M (succ@0 swings 14%↔0%; last clean ckpt ~110–120M @ ~14% @0, backed up at `results/checkpoints/local_max_peak/`). Hypothesis: hard `max` agg (`cellwise_lstm.py:185-210`, `jnp.maximum` accumulate) routes gradient only to the argmax neighbour → sparse gradient → argmax flips at low LR → oscillation. Experiments to run in parallel + log ALL (incl. failures): (1) **re-seed** (systematic vs fluke); (2) **`zero_init_message=true`** (cheap stabiliser); (3) **soft-max** — add a logsumexp/temperature aggregation mode to the cellwise core (β→∞ recovers hard max; finite β flows gradient to all neighbours) = the direct hypothesis test [the attn `maxplus` readout is the attention analog, already running as `attn_masked_200m`]; (4) opt. gentler LR anneal. ~150–200M each. Report whatever happens — "max-agg is unstable here / needs softening" is itself a result.
>
> **✅ EVAL DONE + DATA PULLED — 6×H100 NODE DELETED/DELETABLE (2026-06-11 ~20:10Z).** Full thinking-step sweep (ticks 0→12, valid+train, 1000 levels) pushed to `results/` (commit `764a509`: CSV + figures + `RESULTS.md`). **Headline:** the masked-vs-dense thesis holds *cleanly in the attention instance* — `attn_plain` (masked) 0.377 vs `attn_dense` 0.215 valid@0 (+16pp), masked thinking-lift +4.7pp vs dense +1.3pp (~3.6×). Cellwise arms too weak (~15%) to resolve it behaviourally; `dense_sum` diverges. **All weights pulled** to `results/checkpoints/all_arms/all_ckpts.tar` (3.7GB, 274 ckpts, all 12 arms incl. local_max peak + attn_masked; gitignored) and pkls to `results/data/eval_pkls.tar` (gitignored). **The 6×H100 (157.66.254.38) is being torn down — do NOT ssh it.** Next: mech-interp probes (Tiers 2–4) on a CHEAP single GPU — upload `all_ckpts.tar`, build the JAX activation-extraction + `learned-planner` probe/patch pipeline.
