...
- **Two headline contrasts:** (A) **MASK** local `N` vs dense (all-to-all) → locality enables localized lookahead; (B) **AGGREGATION** `max` (Bellman-optimality = improvement) vs `mean`/`sum` (expectation = evaluation).
- **Generality goal:** show it in non-conv instances (GNN, attention); strongest = a **non-grid** transition graph (conv can't apply).
